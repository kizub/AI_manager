...
// 🔥 BOOTSTRAP ROUTE (КРИТИЧНО — СТОЇТЬ ПЕРЕД ВСІМ)
...
try {
  ...
  if (!user.owner) throw new Error('Failed to create owner');

  let project = await prisma.project.findFirst({
    where: { name: 'Test Project', ownerId: user.owner.id },
  });

  if (!project) {
    project = await prisma.project.create({
      data: { name: 'Test Project', ownerId: user.owner.id },
    });
  }
